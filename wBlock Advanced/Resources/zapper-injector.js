...
<body>
    ...
    /**
     * Get the zapper JavaScript code
     */
    getZapperJavaScript() {
        return `
/**
...
class WBlockElementZapper {
    ...
    setupEventListeners() {
        ...
        if (togglePickerBtn) togglePickerBtn.addEventListener('click', () => this.togglePickerPanel());
        ...
        if (selectorInput) selectorInput.addEventListener('input', () => this.onSelectorInput());
        if (previewBtn) previewBtn.addEventListener('click', () => this.previewSelector());
        if (createRuleBtn) createRuleBtn.addEventListener('click', () => this.createRule());
    ...
    showHighlight(element) {
        ...
        const overlay = document.getElementById('wblock-zapper-overlay');
        if (overlay) {
    ...
    quit() {
        ...
        // Send quit message to parent frame
        if (window.parent && window.parent !== window) {
            window.parent.postMessage({ action: 'zapperQuit' }, '*');
        }
...
// Listen for activation from the Safari extension
if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.wblockAdvanced) {
    // Override message handler to listen for zapper activation
...
// Alternative: Listen for keyboard shortcut (Cmd+Shift+Z or similar)
