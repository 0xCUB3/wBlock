...
let sitePath = "wBlock/SiteSettingsView.swift"
...
let site = try source(sitePath)
for symbol in [
    "let isAutoplayAllowed: Bool",
    "dataManager.noAutoplayAllowedSites",
    ...
    "setNoAutoplaySiteAllowed(allowed, onHost: domain)",
    "setNoAutoplaySiteAllowed(false, onHost: domain)",
    "await dataManager.setNoAutoplaySiteAllowed(snapshot.isAutoplayAllowed, onHost: domain)",
    ".disabled(site.isWhitelisted)",
] {
    require(site, symbol, in: sitePath)
}
forbid(site, "HostMatcher.isHostDisabled", in: sitePath)
