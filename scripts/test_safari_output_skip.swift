...
require(conversion.contains("finalizeContentBlockerJSON"), "conversion must cap ignore-rule injection at Safari's limit")
require(!conversion.contains("baseCount + sitesToUse.count"), "conversion must not report uncapped ignore-rule totals")
require(!conversion.contains("result.safariRulesCount + sitesToUse.count"), "conversion must not report uncapped converter totals")
...
require(fastUpdate.contains("finalizeContentBlockerJSON"), "fast updates must cap ignore-rule injection at Safari's limit")
require(!fastUpdate.contains("let finalRuleCount = baseCount + sitesToUse.count"), "fast updates must not report uncapped ignore-rule totals")
