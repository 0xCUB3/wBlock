...
let body = handler.components(separatedBy: "private static func handleMaybeUpdateUserScripts")[1]
    .components(separatedBy: "private static func handleMaybeStageFilterUpdates")[0]
...
require(!body.contains("maybeRunAutoUpdate") && !body.contains("refreshFiltersIfNeeded") && !body.contains("stageFilterDownloadsFromExtension"), "the userscript ping never runs the filter pipeline (filters stage separately, #528)")
...
require(source.contains("if (changeInfo && changeInfo.status === \"complete\") {\n          maybeRefreshUserScripts().then(maybeStageFilterUpdates);"), "ping fires after page completion")
