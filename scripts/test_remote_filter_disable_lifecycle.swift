...
func runApply(_ initial: FilterState, previouslyApplied: Bool, succeeds: Bool) -> FilterState {
    var state = initial
    let generatedFromCurrentSelection = state.selected
    guard succeeds, previouslyApplied, !generatedFromCurrentSelection else { return state }
