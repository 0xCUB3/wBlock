/*
 * wBlock content runtime.
 * Bootstrap that asks the background/native runtime for advanced rules and
 * injects uBO-compatible scriptlets into the page world.
 */
'use strict';
...
function wBlockPageScriptText(configuration) {
  return `(${wBlockApplyConfiguration.toString()})(${JSON.stringify(configuration || {})});`;
}

function wBlockCanInjectPageScript() {
  return location.protocol === 'http:' || location.protocol === 'https:';
}

function wBlockHasPageExecutableConfiguration(configuration) {
  return !!configuration && (
    Array.isArray(configuration.scriptlets) && configuration.scriptlets.length > 0 ||
    Array.isArray(configuration.js) && configuration.js.length > 0 ||
    Array.isArray(configuration.extendedCss) && configuration.extendedCss.length > 0
  );
}

function wBlockInjectContentCss(configuration) {
  const css = configuration && Array.isArray(configuration.css) ? configuration.css : [];
  if (!css.length || !document.documentElement || !wBlockCanInjectPageScript()) return;
  const style = document.createElement('style');
  style.setAttribute('data-wblock-runtime', 'content-css');
  style.textContent = css.map(selector => selector.includes('{') ? selector : `${selector}{display:none!important;}`).join('\n');
  (document.head || document.documentElement).appendChild(style);
}

function wBlockInjectPageConfiguration(configuration) {
  if (!wBlockHasPageExecutableConfiguration(configuration) || !wBlockCanInjectPageScript()) return;
  const code = wBlockPageScriptText(configuration);
  const parent = document.documentElement || document.head || document.body;
  if (!parent) return;
  const script = document.createElement('script');
...
function wBlockApplyEarlyYouTubeCompatibility() {
  try {
    ...
    wBlockInjectPageConfiguration({ scriptlets: WBLOCK_EARLY_YOUTUBE_SCRIPTLETS, css: [], extendedCss: [], js: [] });
  } catch (error) {
...
function wBlockDelayLoadEvents(timeoutMs = 250) {
  const interceptors = [];
  const events = [
    { target: document, name: 'DOMContentLoaded', options: { bubbles: true, cancelable: false } },
    { target: window, name: 'load', options: { bubbles: false, cancelable: false } }
  ];
  for (const item of events) {
    const interceptor = { ...item, intercepted: false };
    interceptor.listener = event => { event.stopImmediatePropagation(); interceptor.intercepted = true; };
    try { item.target.addEventListener(item.name, interceptor.listener, { capture: true }); interceptors.push(interceptor); } catch (_) {}
  }
  ...
  const release = () => {
    ...
    for (const interceptor of interceptors) {
      try { interceptor.target.removeEventListener(interceptor.name, interceptor.listener, { capture: true }); } catch (_) {}
      if (interceptor.intercepted) {
        try { interceptor.target.dispatchEvent(new Event(interceptor.name, interceptor.options)); } catch (_) {}
      }
    }
  };
...
async function wBlockMain() {
  ...
  try {
    ...
    if (response && response.payload) {
      wBlockInjectContentCss(response.payload);
      wBlockInjectPageConfiguration(response.payload);
    }
  } catch (error) {
    try { console.warn('[wBlock] Failed to initialize content runtime:', error); } catch (_) {}
  } finally {
    releaseEvents();
  }
}
