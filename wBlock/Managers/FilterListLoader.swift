...
class FilterListLoader {
    ...
    private let sharedContainerIdentifier =
    "group.com.0xcube.wBlock" // Best practice: Use a constant
    ...
    /// Loads filter lists from UserDefaults or creates default ones
      func loadFilterLists() -> [FilterList] {
          ...
          if let defaults = UserDefaults(suiteName: sharedContainerIdentifier),
             let data = defaults.data(forKey: "filterLists"),
    ...
    func saveFilterLists(_ filterLists: [FilterList]) {
        ...
        if let data = try? JSONEncoder().encode(filterLists) {
            ...
            defaults.synchronize() // Immediate save
        } else {
