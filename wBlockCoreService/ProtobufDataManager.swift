...
private func mergePersistedChanges(
    ...
    mergeStringSet(
        &whitelist.userScriptsDisabledSites,
        baseline: previous.whitelist.userScriptsDisabledSites,
        persisted: persisted.whitelist.userScriptsDisabledSites
    )
    mergeStringSet(
        &whitelist.noAutoplayBlockedSites,
        baseline: previous.whitelist.noAutoplayBlockedSites,
        persisted: persisted.whitelist.noAutoplayBlockedSites
    )
    ...
    public func loadData() async {
        ...
        do {
            ...
            if await diskStore.fileExists(at: dataFileURL) {
                ...
                // Migrate the built-in BPC filter list from gitflic; a user-added list keeps its URL (#871).
                ...
                if let bpcFilterIndex = appData.filterLists.firstIndex(where: { $0.url == oldBpcFilterURL && !$0.isCustom }) {
