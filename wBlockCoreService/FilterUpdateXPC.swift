...
public final class FilterUpdateClient {
    ...
    public enum FilterUpdateStartResult: Sendable {
        case started
        case alreadyRunning
        case unavailable
        case timedOut
    ...
    public func startFilterUpdate(timeout seconds: TimeInterval = 10.0) async -> FilterUpdateStartResult {
        ...
        let timeoutNanoseconds = seconds == 0
            ? UInt64.zero
            : min(UInt64.max, UInt64((seconds * 1_000_000_000).rounded()))

        return await withCheckedContinuation { (cont: CheckedContinuation<FilterUpdateStartResult, Never>) in
            ...
            filterProxy.startFilterUpdate { accepted in
                if markFinished() {
                    connection.invalidate()
                    cont.resume(returning: accepted ? .started : .alreadyRunning)
                }
            }
        }
    }
